...
class OpenVLAForActionPredictionWithValueHead(PrismaticForConditionalGeneration):
    ...
    def __init__(
        ...
        # Optional Q head and optional target Q head (EMA copy).
        self.q_enabled = bool(q_enabled)
        self.q_target_enabled = bool(q_target_enabled) and self.q_enabled
        self.q_detach_backbone = bool(q_detach_backbone)
        if self.q_enabled:
            if self.vh_mode == "a0" or self.vh_mode == "a6":
                self.q_head = ValueHead(config.text_config.hidden_size)
            elif self.vh_mode == "a":
                self.q_head = ValueHead(config.text_config.hidden_size * 7)
            else:
                raise ValueError(f"Unknown value head mode for q_head: {self.vh_mode}")
            if self.q_target_enabled:
    ...
    def has_q_head(self) -> bool:
        return bool(getattr(self, "q_enabled", False))

    def has_target_q_head(self) -> bool:
        return bool(getattr(self, "q_target_enabled", False)) and hasattr(self, "target_q_head")

    @torch.no_grad()
    def sync_target_q_head(self, tau: float = 1.0) -> None:
        if not self.has_target_q_head():
            return
        tau = float(tau)
        tau = max(0.0, min(1.0, tau))
        if tau >= 1.0:
            self.target_q_head.load_state_dict(self.q_head.state_dict(), assign=True)
            return
        online_sd = self.q_head.state_dict()
        target_sd = self.target_q_head.state_dict()
        for name, target_param in target_sd.items():
            target_param.copy_(target_param * (1.0 - tau) + online_sd[name] * tau)

    def _extract_action_hidden_features(self, last_hidden_state: torch.Tensor, action_len: int) -> torch.Tensor:
        if self.vh_mode == "a0":
    ...
    def _compute_q_from_hidden(
        self,
        hidden_features: torch.Tensor,
        use_target: bool = False,
        detach_backbone: Optional[bool] = None,
    ) -> torch.Tensor:
        if not self.has_q_head():
            raise RuntimeError("Q head is not enabled for this model.")
        # Q head must not backprop into the shared backbone/LoRA.
        # Keep this hard-detached for training stability/isolation.
        hidden_features = hidden_features.detach()
        if use_target:
            if not self.has_target_q_head():
                raise RuntimeError("Target Q head is not enabled for this model.")
            return self.target_q_head(hidden_features)
        return self.q_head(hidden_features)
    ...
    def evaluate_action_with_q(
        ...
        q_values = self._compute_q_from_hidden(
            hidden_features,
            use_target=use_target_q,
            detach_backbone=detach_backbone_for_q,
        )  # [B, 1]
        ...
        idxes = labels[:, -action_len - 1: -1].unsqueeze(-1) - (32000 - 256)
        idxes = idxes.to(logprobs_tensor.device)
        ...
        entropy = -(probs_tensor * logprobs_tensor).sum(dim=-1)
    ...
    def predict_action_batch(
        ...
        output = self.generate(
            ...
            # Guard against occasional invalid logits that would make multinomial fail on GPU.
