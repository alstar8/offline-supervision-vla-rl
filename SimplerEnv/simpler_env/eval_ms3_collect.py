...
class Args:
    ...
    """Disable task randomization and background overlay when supported"""
    real2sim_instruction_template: str = DEFAULT_REAL2SIM_LANGUAGE_INSTRUCTION_TEMPLATE
    """Language instruction template for the OpenReal2Sim recorder task"""
...
def main():
    ...
    if args.use_default_task:
        env_config["use_default_task"] = True
    env: BaseEnv = gym.make(**env_config)
    ...
    while valid_count < args.num_episodes:
        ...
        env_reset_options = {"episode_id": torch.arange(args.num_envs) + eps_count}
        env_reset_options["trajectory_instruction"] = args.real2sim_instruction_template
        if args.use_default_task:
            env_reset_options["use_default_task"] = True
