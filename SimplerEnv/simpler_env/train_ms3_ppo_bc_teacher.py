...
class OpenVLAPPOBCTeacher:
    def __init__(self, all_args, policy):
        ...
        self.ppo_grad_norm = self.args.alg_grad_norm_clip
    ...
    def _measure_ppo_bc_grad_alignment(
        self,
        ...
        params_vh = self.policy.params_vh
        total_params = params + params_vh
        ...
        value_vh_grads = torch.autograd.grad(
            value_loss,
            params_vh,
            retain_graph=True,
            create_graph=False,
        ...
        total_loss_for_probe = policy_loss + value_loss - self.ppo_entropy_coef * entropy_loss
        total_grads = torch.autograd.grad(
            total_loss_for_probe,
            total_params,
            retain_graph=True,
            create_graph=False,
            allow_unused=True,
        ...
        value_vh_vec = self._flatten_grads(value_vh_grads)
        total_vec = self._flatten_grads(total_grads)
        eps = 1e-12
        ppo_norm = float(torch.linalg.norm(ppo_vec).detach().item()) if ppo_vec is not None else 0.0
        bc_norm = float(torch.linalg.norm(bc_vec).detach().item()) if bc_vec is not None else 0.0
        value_vh_norm = float(torch.linalg.norm(value_vh_vec).detach().item()) if value_vh_vec is not None else 0.0
        total_preclip_norm = float(torch.linalg.norm(total_vec).detach().item()) if total_vec is not None else 0.0

        out = dict(
            grad_probe_actor_ppo_norm=ppo_norm,
            grad_probe_actor_bc_norm=bc_norm,
            grad_probe_vh_value_norm=value_vh_norm,
            grad_probe_total_preclip_norm=total_preclip_norm,
            grad_probe_total_postclip_norm=min(total_preclip_norm, float(self.ppo_grad_norm)),
        )
        if total_preclip_norm > eps:
            out["grad_probe_total_clip_scale"] = min(1.0, float(self.ppo_grad_norm) / total_preclip_norm)
        else:
            out["grad_probe_total_clip_scale"] = 1.0

        if ppo_vec is None or bc_vec is None or ppo_norm <= eps or bc_norm <= eps:
            return out
    ...
    def train_ppo_step(self, idx, total, batch):
        ...
        if bc_coef > 0.0:
            ...
            # Soft BC / cross-entropy with rebinned teacher targets.
            bc_loss = -(teacher_probs_on_student * student_log_probs_exec).sum(dim=-1).mean()
        ...
        if should_measure_grad_align:
            grad_align_info = self._measure_ppo_bc_grad_alignment(
                ppo_policy_loss,
                bc_loss,
                value_loss,
                entropy_loss,
...
class Runner:
    ...
    def run(self):
        ...
        for episode in range(max_episodes):
            ...
            ep_time = time.time()
            prompts_seen = set()
            steps_before_rollout = start_steps + episode * steps_per_episode
