...
class PutOnPlateInScene25(BaseEnv):
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        self._use_default_task = options.get("use_default_task", False)
        self._initialize_episode_pre(env_idx, options)

        b = self.num_envs

        # rgb overlay
        if self.overlay_images_numpy:
...
class PutOnPlateInScene25MainV3(PutOnPlateInScene25):
    ...
    def _initialize_episode_pre(self, env_idx: torch.Tensor, options: dict):
        ...
        if use_default_task:
            ...
            self.select_carrot_ids = torch.zeros(b, dtype=torch.long, device=self.device)  # [b]
            self.select_plate_ids = torch.zeros(b, dtype=torch.long, device=self.device)
            self.select_overlay_ids = torch.full(
                (b,), overlay_id, dtype=torch.long, device=self.device
            )
            self.select_pos_ids = torch.randint(0, l1, size=(b,), device=self.device)
            self.select_quat_ids = torch.randint(0, l2, size=(b,), device=self.device)
            return
