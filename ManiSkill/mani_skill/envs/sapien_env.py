...
from mani_skill.utils.debug_tracing import trace, trace_enabled
...
def _configure_render_system_capacity(render_system):
    # Some imported robot/material setups need larger material/texture pools.
    # Apply these opportunistically because the exact API surface can differ
    # across SAPIEN builds.
    for attr_name, value in (
        ("max_num_materials", 50000),
        ("max_num_textures", 50000),
    ):
        try:
            if hasattr(render_system, attr_name):
                setattr(render_system, attr_name, value)
        except Exception:
            pass
    for method_name, value in (
        ("set_max_num_materials", 50000),
        ("set_max_num_textures", 50000),
    ):
        try:
            method = getattr(render_system, method_name, None)
            if callable(method):
                method(value)
        except Exception:
            pass
...
class BaseEnv(gym.Env):
    ...
    def _step_action(
        ...
        perf_metrics = getattr(self, "_vr_perf_current_step_metrics", None)

        def _perf_add(metric_name: str, elapsed_s: float):
            if perf_metrics is None:
                return
            perf_metrics[metric_name] = float(perf_metrics.get(metric_name, 0.0)) + elapsed_s * 1000.0
            count_key = f"{metric_name}_count"
            perf_metrics[count_key] = int(perf_metrics.get(count_key, 0)) + 1
        ...
        if set_action:
            ...
            trace("SapienEnv._step_action.input", action=action)
            t0 = time.perf_counter()
            self.agent.set_action(action)
            _perf_add("agent_set_action_ms", time.perf_counter() - t0)
            if self._sim_device.is_cuda():
                t0 = time.perf_counter()
                self.scene.px.gpu_apply_articulation_target_position()
                _perf_add("gpu_apply_articulation_target_position_ms", time.perf_counter() - t0)
                t0 = time.perf_counter()
                self.scene.px.gpu_apply_articulation_target_velocity()
                _perf_add("gpu_apply_articulation_target_velocity_ms", time.perf_counter() - t0)
        t0 = time.perf_counter()
        self._before_control_step()
        _perf_add("before_control_step_ms", time.perf_counter() - t0)
        for sim_substep in range(self._sim_steps_per_control):
            if self.agent is not None:
                t0 = time.perf_counter()
                self.agent.before_simulation_step()
                _perf_add("agent_before_simulation_step_ms", time.perf_counter() - t0)
            t0 = time.perf_counter()
            self._before_simulation_step()
            _perf_add("before_simulation_step_ms", time.perf_counter() - t0)
            t0 = time.perf_counter()
            self.scene.step()
            _perf_add("scene_step_direct_ms", time.perf_counter() - t0)
            if trace_enabled() and self.agent is not None:
                if self.gpu_sim_enabled:
                    t0 = time.perf_counter()
                    self.scene._gpu_fetch_all()
                    _perf_add("trace_gpu_fetch_all_ms", time.perf_counter() - t0)
                if sim_substep in (0, self._sim_steps_per_control - 1):
                    controller_state = self.agent.controller.get_state()
                    trace_fields = dict(
                        sim_substep=np.array([sim_substep]),
                        robot_qpos=self.agent.robot.get_qpos(),
                        robot_qvel=self.agent.robot.get_qvel(),
                    )
                    if hasattr(self.agent, "ee_pose_at_robot_base"):
                        trace_fields["ee_pose_at_robot_base"] = (
                            self.agent.ee_pose_at_robot_base.raw_pose
                        )
                    if "target_qpos" in controller_state:
                        trace_fields["controller_target_qpos"] = controller_state["target_qpos"]
                    if "target_pose" in controller_state:
                        trace_fields["controller_target_pose"] = controller_state["target_pose"]
                    trace("SapienEnv._step_action.post_dynamics", **trace_fields)
            t0 = time.perf_counter()
            self._after_simulation_step()
            _perf_add("after_simulation_step_ms", time.perf_counter() - t0)
        t0 = time.perf_counter()
        self._after_control_step()
        _perf_add("after_control_step_ms", time.perf_counter() - t0)
        if self.gpu_sim_enabled:
            t0 = time.perf_counter()
            self.scene._gpu_fetch_all()
            _perf_add("final_gpu_fetch_all_ms", time.perf_counter() - t0)
        return action
        ...
        if self._sim_device.is_cuda():
            ...
            for scene_idx in range(self.num_envs):
                ...
                scene = sapien.Scene(
                    systems=[
                        physx_system,
                        _configure_render_system_capacity(
                            sapien.render.RenderSystem(self._render_device)
                        ),
                    ]
                )
