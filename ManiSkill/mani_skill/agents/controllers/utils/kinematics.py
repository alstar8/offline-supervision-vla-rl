...
from mani_skill.utils.debug_tracing import trace
...
class Kinematics:
    ...
    def compute_ik(
        ...
        if self.use_gpu_ik:
            ...
            trace(
                "Kinematics.compute_ik.input",
                q0=q0,
                target_pose=target_pose.raw_pose,
                action=torch.zeros_like(q0) if action is None else action,
                active_ancestor_joint_idxs=torch.as_tensor(
                    self.active_ancestor_joint_idxs, device=self.device
                ),
            )
            if not use_delta_ik_solver:
                ...
                trace("Kinematics.compute_ik.output", solution=solution)
                return solution
            else:
                ...
                solution = q0 + delta_joint_pos.squeeze(-1)
                trace(
