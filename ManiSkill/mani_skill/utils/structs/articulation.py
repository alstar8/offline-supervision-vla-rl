...
class Articulation(BaseStruct[physx.PhysxArticulation]):
    ...
    def set_joint_drive_targets(
        ...
        trace(
            ...
            joint_indices=torch.as_tensor([]) if joint_indices is None else joint_indices,
            joint_names=np.asarray([] if joints is None else [joint.name for joint in joints], dtype=object),
            targets=targets,
        )
        if self.scene.gpu_sim_enabled:
