...
PATCH="$ROOT/runs/manual/pick_red_cube_100_v4/runtime_sim_patch.yaml"
...
if ! flock -n 9; then
  echo "collect_until_100_v4 already running" >&2
  exit 0
fi
cd "$ROOT"
...
seed_start=0
round=0
echo "collect_until_100_v4 start $(date -Is) have=$n_have target=$TARGET scene=airy_table_scene14sep26_left_image lighting=tabletop_legacy_effective_reference_scene14sep26_left commit=58f02e2" | tee -a "$OUT/collect.log"
...
while [ "$n_have" -lt "$TARGET" ]; do
  ...
  "$PY" "$ROOT/runs/manual/pick_red_cube_100/harvest_successes.py" "$round_dir" "$OUT" "$TARGET" | tee -a "$OUT/collect.log"
  n_have="$(count)"
