...
SFT_LORA_PATH="${1:-}"
DATABC_NAME="${DATABC_NAME:-DataBC_pick_red_cube}"

if [[ -z "${SFT_LORA_PATH}" ]]; then
  echo "usage: train_databc.sh /path/to/lora_XXXXXX_or_steps_NNNN" >&2
  exit 1
fi
...
echo "DataBC start $(date -Is) name=${DATABC_NAME} vla_load_path=${SFT_LORA_PATH}"
