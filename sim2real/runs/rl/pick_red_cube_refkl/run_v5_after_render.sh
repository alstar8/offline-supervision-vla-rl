#!/usr/bin/env bash
# Resume v5 pipeline after prepared npz exist: TFDS -> SFT -> RefKL.
set -euo pipefail
...
n_prep="$(find "${PREP_DIR}" -maxdepth 1 -name 'episode_*_bg*.npz' 2>/dev/null | wc -l)"
log "resume after render prep=${n_prep} expected=${TARGET_TRAJ}"
...
rm -rf "${DATASETS}/sft_v2"/1.0.0.incomplete* "${DATASETS}/sft_v2/1.0.0"
